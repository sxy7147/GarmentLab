...
from Env.Config.BimanualFrankaConfig import FrankaConfig
...
class DeformableEnv(BaseEnv):
    def __init__(self,deformbaleConfig:DeformableConfig=None,frankaConfig:FrankaConfig=None):
        ...
        # self.recording_camera = Recording_Camera(camera_position=np.array([0.0, 0.5, 8]), camera_orientation=np.array([0, np.pi/2, -np.pi/2]))
        self.recording_camera = Recording_Camera(camera_position=np.array([0.0, 0.5, 8]), camera_orientation=np.array([0, 90, -90]))
        self.recording_camera.initialize(pc_enable=True, segment_prim_path_list=["/World/Deformable/deformable"])
